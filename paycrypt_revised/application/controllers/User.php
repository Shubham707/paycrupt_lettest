...
class User extends CI_Controller 
{
 ...
 //        	$config['smtp_pass'] = 'vision@123';
	...
	// }
    public function checkPassExist($pass)
    {
        $email=$this->session->userdata('userEmail');
        $curentPass=hash('sha256', strtolower($pass));
      if($this->UserManagerModel->passExist($email,$curentPass))
      {
          return TRUE;
      }
      else {
        $this->form_validation->set_message('checkPassExist', 'Current password does not match.');
        return FALSE;
      }
    }
    public function resetPassword()
    {
        ...
        $this->form_validation->set_rules('current_pass', 'Current password', 'required|callback_checkPassExist',array(
      'required'=> 'You have not provided %s.'));
    $this->form_validation->set_rules('newpass', 'New Password', 'required');
        $this->form_validation->set_rules('confirm_new_pass', 'Confirm New Password', 'required');
        if ($this->form_validation->run() == FALSE)
        {
        $this->load->view('dashboard/setting');
        }
        else {
            $pass=hash('sha256', strtolower($this->input->post('confirm_new_pass')));
            $data = array('password'=>$pass);
            if($this->User_model->updateResetPassword($data))
            {
                $this->session->set_flashdata('pass_success', 'Password updated successfully');
          redirect(base_url().'settings','refresh');
            }
          else {
                $this->session->set_flashdata('pass_fail', 'Password not updated');
