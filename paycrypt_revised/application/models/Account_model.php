...
class Account_model extends CI_Model 
{
	...
	{
		 $sql="select a.*,b.*,c.* from security_key a left join coin b on a.coinName=b.coin_name left join crypto_payments c on (c.boxID=a.boxID and c.coinLabel=a.boxName) where a.email='$email'";
	...
	{
		return $this->db->insert('security_key',$value);
	...
	{
		 return $q=$this->db->select('*')->where('email', $value)->get('affiliated')->result();
		 //print_r($q); die();
	...
	{
		return $this->db->insert('monetiser',array('addresssend'=>$value));
	}
